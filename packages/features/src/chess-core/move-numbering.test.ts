...
import { computeMoveNumber, plyFromMoveNumber } from "./move-numbering";
  ...
  describe("when starting as white", () => {
    ...
    test("alternates white/black correctly for consecutive indices", () => {
      const results = Array.from({ length: 6 }, (_, i) =>
        computeMoveNumber(i, false, 1),
      );
      expect(results.map((r) => r.isWhiteMove)).toEqual([
        true,
        false,
        true,
        false,
