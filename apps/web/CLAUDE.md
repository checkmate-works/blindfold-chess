...
| -------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `QueryDeadlineError` as the `cause` of Drizzle's `Failed query: <sql>`           | a query went unanswered for 10s — but the SQL it names may be an innocent bystander; read the two tags below before blaming it                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| `query_deadline.overshoot_ms` tag in the seconds                                 | this instance's Node event loop was blocked (the deadline timer itself fired that late) — the DB is innocent; suspect CPU saturation on the function instance. 2026-08-05: sub-ms queries blew deadlines while `pg_stat_activity` showed the backend in `wait_event = ClientRead`, i.e. Postgres waiting on the app's own unflushed protocol bytes                                                                                                                                                                                                                                                                                                             |
| `query_deadline.overshoot_ms` tag ≈ 0                                            | the timer fired on schedule and the query truly went unanswered — suspect the connection path (pool queue, Supavisor client limit, network) or genuinely slow execution (then a 57014 should also appear; the 30s server-side `statement_timeout` is confirmed forwarded by Supavisor as of 2026-08-05)                                                                                                                                                                                                                                                                                                                                                        |
| `query_deadline.oldest_inflight_ms` tag far past 10s                             | wedged pool slots: the `query_deadline.inflight_oldest` extra lists queries whose awaiters were rejected long ago but which never settled in postgres.js, so their connections still occupy the `max: 10` pool and starve everything behind them. Confirmed in production 2026-08-05: five wedges accumulated on one instance, the oldest 702s (instance freeze staled the sockets). The pool now self-heals — a query still unsettled 5s past its deadline triggers a full pool rebuild, logged as the Sentry warning `db-pool-rebuilt` (see the `@design` note in `src/lib/db/index.ts`); frequent rebuilds mean the staleness source itself needs attention |
| `db-pool-rebuilt` warnings, rare and paired with recovery                        | the self-heal is working as designed; no action beyond noting the frequency (the `db_pool.rebuild_reason` tag says which path fired: `wedged-query` or `deadline-retry`)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| `db-deadline-retry` with `db_retry.outcome: rescued`                             | a SELECT went silent for 10s but its transparent second attempt on a fresh pool succeeded — the user saw a slow page, not an error page. Frequent occurrences carry the same meaning as `db-pool-rebuilt`: the connection path is eating queries (2026-08-05: established connections black-holing sub-ms SELECTs on never-frozen instances — external/Supavisor suspect)                                                                                                                                                                                                                                                                                      |
| `db-deadline-retry` with `db_retry.outcome: failed`                              | both the original SELECT and its retry on a fresh pool went unanswered — the outage spans connections, so suspect the pooler/path as a whole, not one stale socket                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| `query_deadline.inflight_count` ≈ pool size with all ages under ~10s             | plain pool-queue congestion: too many genuinely concurrent queries for 10 connections — a throughput problem, not a wedge                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
