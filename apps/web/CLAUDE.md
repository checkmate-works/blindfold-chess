...
- **Static routes rely on ISR intervals plus cache tags, not on-demand purges** — since 2026-08, ~50 route patterns under `[locale]/` are SSG/ISR (content pages: faq/privacy/terms, glossary, learn, dojo, practice tops/tutorials, articles, …) with a one-day default `revalidate` (set in `[locale]/layout.tsx`; several pages declare their own 30–60 min). Their staleness budget is deliberate: content that must surface sooner is tag-driven — admin announcement mutations invalidate the `announcements` tag, which marks the affected pages stale so each re-renders on its next visit — so mutations still don't need `revalidatePath`. Auth/UGC/feed surfaces remain dynamic (`ƒ`), where there is no Full Route Cache entry to purge. (`src/app/admin/articles/_lib/revalidate-public.ts` predates this and covers the immediate-purge case for article slugs.)
