...
| 提案募集中 / 公開済み         | draft / published (chunk lifecycle)      | `chunks.status` column; `publishChunkEntry` in `src/lib/chunks/user-chunk-mutations.ts` (publish is one-way). Stored values stay `draft` / `published`; **no user-facing string says 下書き / "Draft"** — see the `status` TSDoc on the `chunks` table for why. 下書き in the UI now only ever means the unsaved sessionStorage authoring draft (`ChunkDraftV1`), a different thing that had been sharing the word.           |
