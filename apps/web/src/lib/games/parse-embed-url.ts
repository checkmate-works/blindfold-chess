 ...
 *
 * The length / protocol / userinfo / hostname front half is
 * `parseAllowedHostUrl`, shared with `chesscom-attribution.ts`. See its
 * TSDoc for which attacks the exact-match hostname rule collapses into a
 * single `wrong_host`.
 *
...
export type ParseEmbedUrlResult =
  ...
  | {
      ok: false;
      reason: AllowedHostUrlFailure | 'invalid_path' | 'invalid_id' | 'fragment_not_allowed';
    };
...
export function parseChesscomEmboardUrl(input: string): ParseEmbedUrlResult {
  const parsed = parseAllowedHostUrl(input, {
    hostname: CHESSCOM_HOSTNAME,
