...
const { chunks } = await import('@/lib/db');
const { excludeBlockedAuthors } = await import('@/lib/moderation/block');
const { listChunksWithProfile } = await import('./queries');
...
describe('listChunksWithProfile', () => {
  it('leaves out the authors a signed-in viewer has a block with', async () => {
    ...
    await listChunksWithProfile({ includeDeleted: false, limit: 20, offset: 0, viewerId: VIEWER });

    expect(excludeBlockedAuthors).toHaveBeenCalledWith(chunks.userId, VIEWER);
    expect(lastWhereSql()).toContain('blocked_filter');
  });

  it('asks for no filter on the anonymous catalog', async () => {
    await listChunksWithProfile({ includeDeleted: false, limit: 20, offset: 0 });

    expect(excludeBlockedAuthors).toHaveBeenCalledWith(chunks.userId, undefined);
  });
});
