...
export async function submitEditRequestEntry(params: {
  ...
  // A suggestion is rendered to the chunk's owner (and to anyone reading the
  // chunk's suggestion list) with the proposer's avatar and name, falling back
  // to the "deleted proposer" label when no profile row backs it — see
  // `EditRequestHeader`. So the profile has to exist before the row lands.
  // Accept / reject / withdraw below stay on the plain guard: they resolve a
  // suggestion that already exists rather than attributing a new one.
  const guardResult = await authenticateGuardAndRequireProfile(RATE_LIMITS.submitChunkEditRequest);
