...
const { games } = await import('@/lib/db');
const { excludeBlockedAuthors } = await import('@/lib/moderation/block');
const { listSharedGames } = await import('./games-read');
...
describe('listSharedGames', () => {
  it('leaves out the authors a signed-in viewer has a block with', async () => {
    ...
    await listSharedGames('new', 20, 0, VIEWER);

    expect(excludeBlockedAuthors).toHaveBeenCalledWith(games.authorId, VIEWER);
    expect(lastWhereSql()).toContain('blocked_filter');
  });

  it('asks for no filter on the anonymous gallery', async () => {
    await listSharedGames('new', 20, 0);

    expect(excludeBlockedAuthors).toHaveBeenCalledWith(games.authorId, undefined);
  });
});
