...
/**
 * Chunk Themes — junction between chunks (piece-coordination patterns)
 * and the glossary terms (themes) that they illustrate.
 *
 * @design parallel to position_themes, anchored on the chunk
 * Where `position_themes` tags a `positions` row with a glossary term,
 * `chunk_themes` tags a `chunks` row. The shape is identical and the
 * RLS policy mirror is intentional — both junctions are owner-gated
 * (the owner of the tagged thing controls the link), both gate INSERT
 * on `glossary_terms.is_theme = true`, and both expose public SELECT.
 *
 * The pair (`chunk_themes`, `position_themes`) exists rather than a
 * polymorphic `theme_targets(target_type, target_id, term_id)` for the
 * same reason `(position_chunks, position_themes)` are separate today:
 * referential integrity stays strict, RLS stays trivially expressible,
 * and each side can evolve its lifecycle (visibility, proposals,
 * voting) without affecting the other.
 *
 * @design distinct from glossary_term_positions
 * `glossary_term_positions` is master-data "here is an example FEN
 * illustrating this term" (curated visual examples, lives in seed).
 * `chunk_themes` is "this chunk (a named piece-coordination pattern)
 * illustrates this term" (a directed relationship between two
 * catalog rows). The two are complementary: a term page can show
 * "examples" (positions) and "patterns" (chunks) side by side.
 *
 * @design ON DELETE RESTRICT on term_id
 * Same rationale as `position_themes`: master-data terms are not
 * normally deleted; the RESTRICT blocks accidental physical deletes
 * while the chunk's logical delete (`deletedAt`) is the normal
 * deprecation path on the chunk side.
 *
 * @design is_theme gate enforced in RLS
 * Same INSERT predicate as `position_themes`: only `glossary_terms`
 * rows with `is_theme = true` may be attached. The application's theme
 * picker filters identically, and the DB-level WITH CHECK re-asserts
 * the rule so a direct REST write cannot bypass it.
 *
 * Junction tables have no `updated_at` by convention.
 */
export const chunkThemes = pgTable(
  ...
  (table) => [
    primaryKey({ columns: [table.chunkId, table.termId] }),
    // Reverse lookup index: "chunks tagged with term X".
    index('idx_chunk_themes_term').on(table.termId),
  ]
);

export type ChunkTheme = typeof chunkThemes.$inferSelect;
export type NewChunkTheme = typeof chunkThemes.$inferInsert;
