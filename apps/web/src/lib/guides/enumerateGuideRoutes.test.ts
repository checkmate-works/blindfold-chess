...
describe('enumerateGuideRoutes', () => {
  it('emits a root entry for every rank that has guide content', () => {
    ...
    // Real i18n data currently has mukyu, 5kyu, 4kyu, 3kyu as flat guides.
    expect(roots).toEqual(expect.arrayContaining(['mukyu', '5kyu', '4kyu', '3kyu']));
    // Ranks without guide data must be absent.
    expect(roots).not.toContain('2kyu');
