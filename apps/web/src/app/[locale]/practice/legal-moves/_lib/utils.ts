...
// Generate a sequence of move questions
export function generateMoveQuestions(count: number): MoveQuestion[] {
  const questions: MoveQuestion[] = [];
  const pieces: PieceType[] = ['bishop', 'knight', 'rook', 'queen', 'king'];
  ...
  while (questions.length < count) {
    ...
    // Random from and to squares
    const fromSquare = squares[Math.floor(Math.random() * squares.length)];
    let toSquare = squares[Math.floor(Math.random() * squares.length)];

    // Ensure from and to are different
    while (toSquare === fromSquare) {
      toSquare = squares[Math.floor(Math.random() * squares.length)];
    }

    // Check if this question already exists
    const exists = questions.some(
      (q) => q.from === fromSquare && q.to === toSquare && q.piece === piece
    );
...
export function generateBalancedMoveQuestions(
  ...
  while (questions.length < count) {
    const piece = allowedPieces[Math.floor(Math.random() * allowedPieces.length)];
    const question = generateMoveQuestionForPiece(piece, legalCount < targetLegalCount);
...
function generateMoveQuestionForPiece(
  pieceType: PieceType,
  preferLegal: boolean
): MoveQuestion | null {
  const files = ['a', 'b', 'c', 'd', 'e', 'f', 'g', 'h'];
