...
// English strings keep the existing assertions readable; label resolution
// itself is covered by the dedicated labels test below.
const labels = {
  navLabel: 'Pagination',
  previous: 'Previous',
  next: 'Next',
...
describe('PaginationNavView', () => {
  it('renders nothing when totalPages is 1', () => {
    const { container } = render(
      <PaginationNavView labels={labels} currentPage={1} totalPages={1} buildHref={buildHref} />
    );
  ...
  it('renders nothing when totalPages is 0', () => {
    const { container } = render(
      <PaginationNavView labels={labels} currentPage={1} totalPages={0} buildHref={buildHref} />
    );
    expect(container.innerHTML).toBe('');
  });
  ...
  it('has a nav element with aria-label', () => {
    render(
      <PaginationNavView labels={labels} currentPage={1} totalPages={2} buildHref={buildHref} />
    );
  ...
  describe('Previous/Next buttons', () => {
    it('disables Previous on first page', () => {
      render(
        <PaginationNavView labels={labels} currentPage={1} totalPages={3} buildHref={buildHref} />
      );
    ...
    it('enables Previous on non-first page', () => {
      render(
        <PaginationNavView labels={labels} currentPage={2} totalPages={3} buildHref={buildHref} />
      );
      const previous = screen.getByLabelText('Previous page');
    ...
    it('disables Next on last page', () => {
      render(
        <PaginationNavView labels={labels} currentPage={3} totalPages={3} buildHref={buildHref} />
      );
  ...
  describe('page number links', () => {
    it('renders page numbers as links except for current page', () => {
      render(
        <PaginationNavView labels={labels} currentPage={2} totalPages={5} buildHref={buildHref} />
      );
    ...
    it('highlights the current page with aria-current', () => {
      render(
        <PaginationNavView labels={labels} currentPage={3} totalPages={5} buildHref={buildHref} />
      );
    ...
    it('shows ellipsis when total is 7', () => {
      render(
        <PaginationNavView labels={labels} currentPage={4} totalPages={7} buildHref={buildHref} />
      );
  ...
  describe('truncation with ellipsis', () => {
    it('shows ellipsis for large page counts when on first page', () => {
      render(
        <PaginationNavView labels={labels} currentPage={1} totalPages={10} buildHref={buildHref} />
      );
    ...
    it('shows ellipsis for large page counts when on last page', () => {
      render(
        <PaginationNavView labels={labels} currentPage={10} totalPages={10} buildHref={buildHref} />
      );
    ...
    it('shows two ellipses when current page is in the middle of a large set', () => {
      render(
        <PaginationNavView labels={labels} currentPage={5} totalPages={10} buildHref={buildHref} />
      );
    ...
    it('does not show leading ellipsis when window touches first page', () => {
      render(
        <PaginationNavView labels={labels} currentPage={2} totalPages={10} buildHref={buildHref} />
      );
    ...
    it('does not show trailing ellipsis when window touches last page', () => {
      render(
        <PaginationNavView labels={labels} currentPage={9} totalPages={10} buildHref={buildHref} />
      );
  ...
  describe('edge cases', () => {
    ...
    it('renders with exactly 2 pages on last page', () => {
      render(
        <PaginationNavView labels={labels} currentPage={2} totalPages={2} buildHref={buildHref} />
      );
      const previous = screen.getByLabelText('Previous page');
      const next = screen.getByLabelText('Next page');
      expect(previous.tagName).toBe('A');
  ...
  describe('truncation boundary at 6 pages (smallest truncated set)', () => {
    it('shows trailing ellipsis only when on page 3 of 6 (middle)', () => {
      render(
        <PaginationNavView labels={labels} currentPage={3} totalPages={6} buildHref={buildHref} />
      );
    ...
    it('shows trailing ellipsis only when on page 1 of 6', () => {
      render(
        <PaginationNavView labels={labels} currentPage={1} totalPages={6} buildHref={buildHref} />
      );
    ...
    it('shows leading ellipsis only when on page 6 of 6', () => {
      render(
        <PaginationNavView labels={labels} currentPage={6} totalPages={6} buildHref={buildHref} />
      );
    ...
    it('shows leading ellipsis only when on page 4 of 6', () => {
      render(
        <PaginationNavView labels={labels} currentPage={4} totalPages={6} buildHref={buildHref} />
      );
  ...
  describe('boundary: totalPages=5 (last non-truncated case)', () => {
    it('shows all pages when on first page (currentPage=1)', () => {
      render(
        <PaginationNavView labels={labels} currentPage={1} totalPages={5} buildHref={buildHref} />
      );
      for (let i = 1; i <= 5; i++) {
        expect(screen.getByText(String(i))).toBeInTheDocument();
      }
    ...
    it('shows all pages when on last page (currentPage=5)', () => {
      render(
        <PaginationNavView labels={labels} currentPage={5} totalPages={5} buildHref={buildHref} />
      );
      for (let i = 1; i <= 5; i++) {
        expect(screen.getByText(String(i))).toBeInTheDocument();
      }
  ...
  describe('boundary: totalPages=6 near start and end', () => {
    it('shows correct pages when on page 2 of 6 (near start)', () => {
      render(
        <PaginationNavView labels={labels} currentPage={2} totalPages={6} buildHref={buildHref} />
      );
    ...
    it('shows correct pages when on page 5 of 6 (near end)', () => {
      render(
        <PaginationNavView labels={labels} currentPage={5} totalPages={6} buildHref={buildHref} />
      );
  ...
  describe('labels prop', () => {
    it('renders all five labels from the labels prop (localised strings)', () => {
      render(
        <PaginationNavView
          labels={{
            navLabel: 'ページ送り',
