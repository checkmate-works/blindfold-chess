 ...
 *
 * @flow
 * 1. Resolve parent puzzle (404 if missing / soft-deleted)
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const parent = await getPositionById({ id, type: 'puzzle' });
  if (!parent) return { title: t('forksList.title') };
  const title = t('forksList.titleOf', { parentTitle: parent.title });
  return {
    ...generateCanonicalMetadata({ locale, path: `practice/puzzle/${id}/forks`, title }),
    title: resolveTitle(title, locale),
  };
}

export default async function PuzzleForksListPage({ params, searchParams }: Props) {
  const { locale, id } = await params;
  ...
  const totalCount = await countPositions({ type: 'puzzle', forkedFromId: id });
  ...
  const buildHref = (p: number) => {
    const qs = p > 1 ? `?page=${p}` : '';
    return `/${locale}/practice/puzzle/${id}/forks${qs}`;
  };
  ...
  return (
    <PageLayout
      ...
      breadcrumb={[
        ...
        { label: parent.title, href: `/practice/puzzle/${id}` },
