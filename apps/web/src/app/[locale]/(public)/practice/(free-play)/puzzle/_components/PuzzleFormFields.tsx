...
import type { PuzzleFormPhase } from '../_hooks/use-puzzle-form-composition';
...
type Props = {
  ...
  phase: PuzzleFormPhase;
  onPhaseChange: (phase: PuzzleFormPhase) => void;
  title: string;
 ...
 *
 * The board editor and the solution-move editor are two mutually exclusive
 * phases (`phase` prop): the board is only editable during `'position'`, and
 * the solution-move list is only editable during `'solution'` (the board
 * renders read-only there instead). Moving from `'solution'` back to
 * `'position'` — the only direction that can invalidate already-entered
 * moves — goes through an explicit confirmation when there is anything to
 * lose, via `handleConfirmEditPosition` below.
 *
...
export function PuzzleFormFields({
  ...
  const [editPositionConfirmOpen, setEditPositionConfirmOpen] = useState(false);

  const reachedMaxMoves = solution.moves.length >= MAX_SOLUTION_MOVES;

  function handleContinueToSolution() {
    if (!board.isFenValid) return;
    onPhaseChange('solution');
  }

  function handleRequestEditPosition() {
    if (solution.moves.length > 0) {
      setEditPositionConfirmOpen(true);
      return;
    }
    onPhaseChange('position');
  }

  // Explicit clear-to-empty (not solution.reset(), which would fall back to
  // initialMoves — the puzzle's originally saved moves on the edit form).
  // Once the author has chosen to rework the position, moves entered against
  // the old position are equally stale whether they came from the DB or from
  // this session, so both should go.
  function handleConfirmEditPosition() {
    setEditPositionConfirmOpen(false);
    solution.setMoves([]);
    solution.setNotes([]);
    solution.setMoveInput('');
    solution.setMoveError(null);
  ...
  return (
    <>
      ...
      {phase === 'position' && (
        <>
          ...
          <Button
            ...
            variant="primary"
            fullWidth
            disabled={!board.isFenValid}
            onClick={handleContinueToSolution}
          >
            {t('continueToSolution')}
          </Button>
        </>
      )}
      ...
      {phase === 'solution' && (
        <>
          ...
          <div className="flex justify-center">
            <button
              type="button"
              onClick={handleRequestEditPosition}
              className="px-3 py-1 text-sm rounded border border-border text-muted-foreground hover:bg-muted transition-colors"
            >
              {t('editPosition')}
            </button>
      ...
      <ConfirmationModal
        isOpen={editPositionConfirmOpen}
        title={t('editPositionConfirmTitle')}
        message={t('editPositionConfirmMessage')}
        confirmText={t('editPositionConfirmConfirm')}
        cancelText={t('editPositionConfirmCancel')}
        confirmVariant="danger"
        onConfirm={handleConfirmEditPosition}
        onCancel={() => setEditPositionConfirmOpen(false)}
      />
    </>
  );
}
