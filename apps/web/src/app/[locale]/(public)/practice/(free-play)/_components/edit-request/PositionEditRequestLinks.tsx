...
function suggestionsPath(positionType: PositionType, id: string): string | null {
  switch (positionType) {
    case 'memory':
      return `/practice/position-memory/${id}/suggestions`;
    case 'puzzle':
      return `/practice/puzzle/${id}/suggestions`;
    case 'sequence':
      // No detail / suggestions page for sequences.
      return null;
    default: {
...
export async function PositionEditRequestSummaryLink({
  ...
  return (
    ...
    >
      ...
      {t('callout.suggestionsLink', { count: totalCount })}
      {pendingCount > 0 && (
