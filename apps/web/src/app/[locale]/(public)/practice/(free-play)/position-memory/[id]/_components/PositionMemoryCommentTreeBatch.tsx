...
/**
 * One rendered batch of the position-memory comment tree:
 * `buildCommentTree` + the attachment node map + `<CommentTree>` with the
 * position-memory page's action wiring (no spoilers — there is no
 * solution to hide).
 *
 * Extracted from the page so the SSR'd first batch and every batch
 * returned by the `loadMorePositionMemoryComments` Server Action render
 * through the SAME component — the incremental batches cannot drift from
 * what the page shows (issue #81's uniformity requirement, applied
 * within the page).
 */
export async function PositionMemoryCommentTreeBatch({
  locale,
  positionId,
  userId,
  comments,
