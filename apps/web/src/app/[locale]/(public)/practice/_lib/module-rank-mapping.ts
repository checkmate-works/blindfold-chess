 ...
 * the practice module `menuType` whose card represents submitting that kind
 * of position. Lets such ranks surface on every matching practice card the
 * same way `challenge_score` ranks do.
 *
 * - `memory` → `position_memory`, `puzzle` → `puzzle`: 2kyu is earned by
 ...
 * - a `position_submission_count` requirement whose `positionTypes` includes a
 *   type that maps to this `menuType` (see {@link POSITION_TYPE_TO_MENU_TYPE}).
 *
  ...
  const found = sorted.find((rank) =>
    rank.requirements.some((req) => {
      ...
      if (req.type === 'position_submission_count')
        return req.positionTypes.some((t) => POSITION_TYPE_TO_MENU_TYPE[t] === menuType);
      return false;
    })
  );
