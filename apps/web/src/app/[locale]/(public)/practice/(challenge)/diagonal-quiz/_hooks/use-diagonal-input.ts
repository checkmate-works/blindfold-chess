...
export function useDiagonalInput({
  ...
  const [antiDiagonal, setAntiDiagonal] = useState<DiagonalInputState>(INITIAL_STATE);
  ...
  const isAntiDiagonalComplete =
    ...
    (allowSingleSquareAntiDiagonal && isSingleSquareComplete(antiDiagonal.chars));
  ...
  const handleFilePress = useCallback(
    (file: string) => {
      ...
      const allowSingleCurrent =
        activeField === 'diagonal' ? allowSingleSquareDiagonal : allowSingleSquareAntiDiagonal;
      const allowSingleOther =
        activeField === 'diagonal' ? allowSingleSquareAntiDiagonal : allowSingleSquareDiagonal;
      ...
      const currentComplete =
        isComplete(newChars) || (allowSingleCurrent && isSingleSquareComplete(newChars));
      const otherComplete =
  ...
  useEffect(() => {
    if (activeField === 'diagonal' && isDiagonalComplete && !isAntiDiagonalComplete) {
      setActiveField('antiDiagonal');
    }
