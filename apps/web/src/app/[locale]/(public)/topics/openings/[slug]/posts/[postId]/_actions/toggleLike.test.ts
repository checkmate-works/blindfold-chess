...
describe('toggleLike (openings)', () => {
  ...
  describe('input validation', () => {
    it('should return invalidPostId for non-UUID postId', async () => {
      const result = await toggleLike('not-a-uuid', 'en', 'sicilian-defense');
      expect(result).toEqual({ error: 'invalidPostId' });
      expect(mockGetUser).not.toHaveBeenCalled();
    });

    it('should return invalidOpening for invalid opening slug', async () => {
      mockIsValidOpening.mockResolvedValue(false);

      const result = await toggleLike(testPostId, 'en', 'nonexistent-opening');
  ...
  describe('authentication', () => {
    it('should return signInRequired when user is not authenticated', async () => {
      ...
      const result = await toggleLike(testPostId, 'en', 'sicilian-defense');
  ...
  describe('like toggle', () => {
    ...
    it('should rethrow non-unique-violation errors', async () => {
      ...
      await expect(toggleLike(testPostId, 'en', 'sicilian-defense')).rejects.toThrow(
        'Connection failed'
      );
    });
  });
  ...
  describe('notification to post author', () => {
    ...
    it('should create notification for post author when liking another user post', async () => {
      ...
      await toggleLike(testPostId, 'en', 'sicilian-defense');
      ...
      expect(createNotification).toHaveBeenCalledWith({
        ...
        targetId: testPostId,
        metadata: { topicType: 'opening', topicKey: 'sicilian-defense', postId: testPostId },
      });
    });
  ...
  describe('validation order', () => {
    it('should validate postId before checking opening', async () => {
      mockIsValidOpening.mockResolvedValue(false);
      const result = await toggleLike('not-a-uuid', 'en', 'nonexistent-opening');
      expect(result).toEqual({ error: 'invalidPostId' });
    });

    it('should validate opening before checking auth', async () => {
      mockIsValidOpening.mockResolvedValue(false);
      const result = await toggleLike(testPostId, 'en', 'nonexistent-opening');
      expect(result).toEqual({ error: 'invalidOpening' });
      expect(mockGetUser).not.toHaveBeenCalled();
