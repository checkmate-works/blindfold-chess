...
import { isMaiaExempt } from '@/lib/users/can-use-maia';
 ...
 * route, only when the selected engine is Maia and the viewer is not
 * Maia-exempt. Subscribers short-circuit to `{ charged: false }`.
 *
...
export async function startMaiaGame(clientGameId: string): Promise<StartMaiaGameResult> {
  ...
  // Subscribers play Maia for free — no per-game point charge.
  if (await isMaiaExempt(auth.user.id)) {
